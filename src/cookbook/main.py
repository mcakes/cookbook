...
def generate(
    ...
    template: typing.Annotated[
        Path, typer.Option(
        "-t", "--template", help="Path to the recipe template file."
    )] = Path("templates/recipe.html"),
    output_dir: typing.Annotated[
        Path, typer.Option(
        "-o", "--output-dir", help="Directory to save generated recipes."
    )] = Path("www")
):
    ...
    with open(template, "r") as template_file:
        for path in paths:
            with open(path, "r") as recipe_file:
                ...
                output = generate_recipe(template_file, parsed_recipe)
